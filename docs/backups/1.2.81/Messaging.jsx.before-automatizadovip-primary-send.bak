 ...
 * Función: Módulo Mensajería: plantillas de WhatsApp, envío Web/Móvil y envío servidor mediante AutomatizadoVIP.
...
import { MessageSquare, Send, Copy, Check, MessageCircle } from "lucide-react";
import { toast } from "sonner";
import AutomatizadoVIPSendPanel from "./AutomatizadoVIPSendPanel";
  ...
  return (
    <div className="messaging-page space-y-6 animate-in fade-in duration-200">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">Envío manual, envío automático mediante AutomatizadoVIP e historial de comunicaciones.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="messaging-composer lg:col-span-8 bg-slate-900/90 border border-slate-800 rounded-xl p-5 shadow-xl space-y-4">
          ...
          <div>
            ...
            <textarea rows="6" value={customMessage} onChange={(e) => setCustomMessage(e.target.value)} className="messaging-field messaging-message w-full p-3.5 bg-slate-950 border border-slate-700 rounded-xl text-xs text-slate-100 focus:outline-none focus:border-emerald-500 font-sans leading-relaxed"></textarea>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-3 pt-3 border-t border-slate-800">
            <button onClick={handleCopy} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-xl flex items-center gap-1.5 transition">{copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}{copied ? "Copiado" : "Copiar Texto"}</button>
            <button onClick={handleSendWhatsApp} className="px-5 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white text-xs font-bold rounded-xl flex items-center gap-2 shadow-lg shadow-emerald-600/20 transition"><Send className="w-4 h-4" /> Enviar por WhatsApp Web / Móvil</button>
          </div>
          <AutomatizadoVIPSendPanel client={selectedClient} defaultMessage={customMessage} />
        </div>
      </div>
