...
export default function AutomatizadoVIPSendPanel({ client, defaultMessage = "" }) {
  ...
  const send = async () => {
    ...
    try {
      const res = await axios.post(`${API}/whatsapp/automatizadovip/send`, {
        contacts: [{ client_id: client.id, number: client.phone, message }],
  ...
  return (
    <div className="rounded-xl border border-emerald-500/20 bg-emerald-950/10 p-3 space-y-3">
      <div className="flex items-center gap-2 text-xs font-bold text-emerald-300"><MessageCircle className="w-4 h-4" /> AutomatizadoVIP</div>
      ...
      <button onClick={send} disabled={sending} className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-bold flex items-center gap-2"><Send className="w-4 h-4" /> {sending ? "Enviando..." : "Enviar automáticamente"}</button>
    </div>
  );
}
