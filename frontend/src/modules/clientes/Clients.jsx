 ...
 * Trabaja con: backend/app/routers/clientes/router.py (/api/clients), backend/app/integrations/mikrotik/service.py, modules/planes, modules/red, CoordinatesPicker.jsx
 */
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <button
            ...
            onClick={() => {
              ...
              setFormData({
                ...
                pppoe_user: "", pppoe_password: "", plan_id: activePlans[0]?.id || "",
                router_id: mikrotikRouters[0]?.id || "", ipv4_network_id: "", nap_box: "", nap_box_id: "",
                nap_port: "", optical_power_dbm: "", installation_date: "", technology: "fiber",
                zone_id: "", zone_name: "", monitoring_equipment_id: "", monitoring_equipment_name: "",
                antenna_type: "", management_ip: "", status: "active", billing_day: new Date().getDate(),
                billing_type: "prepaid", invoice_lead_days: 5, grace_days: 5, cut_after_months: 1,
                invoice_notification_channel: "none", payment_reminder_channel: "none",
                reminder_1_days: null, reminder_2_days: null, reminder_3_days: null, create_first_invoice: true
              });
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-xl flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:w-96">
          ...
          <input data-testid={TEST_IDS.CLIENT_SEARCH} type="text" placeholder="Buscar por nombre, DNI/RUC, IP, teléfono o dirección..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500" />
        </div>
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
          <button onClick={() => setStatusFilter("all")} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${statusFilter === "all" ? "bg-cyan-500 text-white" : "bg-slate-800 text-slate-400 hover:bg-slate-700"}`}>Todos ({clients.length})</button>
          <button onClick={() => setStatusFilter("active")} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${statusFilter === "active" ? "bg-emerald-500 text-white" : "bg-slate-800 text-slate-400 hover:bg-slate-700"}`}>Activos</button>
          <button onClick={() => setStatusFilter("suspended")} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${statusFilter === "suspended" ? "bg-rose-500 text-white" : "bg-slate-800 text-slate-400 hover:bg-slate-700"}`}>Suspendidos</button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-300">
              {loading ? (
                <tr><td colSpan="6" className="py-8 text-center text-slate-500">Cargando abonados...</td></tr>
              ) : clients.length === 0 ? (
                <tr><td colSpan="6" className="py-8 text-center text-slate-500">No se encontraron abonados con los filtros aplicados.</td></tr>
              ) : (
                clients.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-800/40 transition">
                    <td className="py-3 px-4">
                      ...
                      <div className="text-[10px] text-slate-400 truncate max-w-xs mt-0.5 flex items-center gap-1">
                        {c.address ? <button type="button" onClick={() => setLocationClient(c)} title={c.latitude && c.longitude ? "Ver ubicación en el mapa" : "El abonado no tiene coordenadas registradas"} disabled={!c.latitude || !c.longitude} className={`inline-flex items-center gap-1 text-left truncate max-w-xs transition ${c.latitude && c.longitude ? "text-cyan-300 hover:text-cyan-200 hover:underline cursor-pointer" : "text-slate-400 cursor-default"}`}><MapPin className="w-2.5 h-2.5 shrink-0" />{c.address}</button> : <span className="text-slate-500">Sin dirección registrada</span>}
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4"><div className="font-mono text-slate-200 cursor-pointer hover:text-cyan-300" onClick={() => window.open(`http://${c.ip_address}`, "_blank")} title="Abrir MikroTik / equipo en una nueva pestaña">{c.ip_address}</div><div className="text-[11px] text-slate-400">{c.connection_type}: <span className="font-mono text-cyan-400">{c.pppoe_user || "estática"}</span></div><div className="text-[10px] text-slate-500">Router: {c.router_name}</div></td>
                    ...
                    <td className="py-3 px-4 text-center"><div className="flex items-center justify-center gap-1.5">
                      <button onClick={() => setDetailClientId(c.id)} title="Ver ficha del cliente" className="p-1.5 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 transition"><ExternalLink className="w-4 h-4" /></button>
                      <button data-testid={`btn-toggle-client-${c.id}`} onClick={() => handleToggleStatus(c.id, c.full_name, c.status)} title={c.status === "active" ? "Cortar Servicio MikroTik" : "Reactivar Servicio"} className={`p-1.5 rounded-lg border transition ${c.status === "active" ? "bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border-rose-500/30" : "bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border-emerald-500/30"}`}><ShieldAlert className="w-4 h-4" /></button>
                      {c.onu_sn && <button data-testid={`btn-onu-client-${c.id}`} onClick={() => handleOnuStatus(c)} title="Ver ONU en la OLT (estado y potencia óptica)" className="p-1.5 rounded-lg bg-cyan-600/10 hover:bg-cyan-600/20 text-cyan-300 border border-cyan-600/30 transition"><Radio className="w-4 h-4" /></button>}
                      <button data-testid={`btn-whatsapp-client-${c.id}`} onClick={() => openWhatsAppReminder(c)} title="Enviar aviso WhatsApp" className="p-1.5 rounded-lg bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-400 border border-emerald-600/30 transition"><MessageSquare className="w-4 h-4" /></button>
                      <button data-testid={`btn-edit-client-${c.id}`} onClick={() => { setSelectedClient(c); setFormData({ full_name: c.full_name, dni_ruc: c.dni_ruc, phone: c.phone, email: c.email || "", address: c.address, reference: c.reference || "", latitude: c.latitude ?? "", longitude: c.longitude ?? "", ip_address: c.ip_address, onu_sn: c.onu_sn || "", connection_type: c.connection_type || "PPPoE", pppoe_user: c.pppoe_user || "", pppoe_password: c.pppoe_password || "", plan_id: c.plan_id, router_id: c.router_id, ipv4_network_id: c.ipv4_network_id || "", nap_box: c.nap_box || "", nap_box_id: c.nap_box_id || "", nap_port: c.nap_port ?? "", optical_power_dbm: c.optical_power_dbm ?? "", installation_date: c.installation_date || "", technology: c.technology || "fiber", zone_id: c.zone_id || "", zone_name: c.zone_name || "", monitoring_equipment_id: c.monitoring_equipment_id || "", monitoring_equipment_name: c.monitoring_equipment_name || "", antenna_type: c.antenna_type || "", management_ip: c.management_ip || "", status: c.status, billing_day: c.billing_day ?? 5, billing_type: c.billing_type || "prepaid", invoice_lead_days: c.invoice_lead_days ?? 5, grace_days: c.grace_days ?? 5, cut_after_months: c.cut_after_months ?? 1, invoice_notification_channel: c.invoice_notification_channel || "none", payment_reminder_channel: c.payment_reminder_channel || "none", reminder_1_days: c.reminder_1_days ?? null, reminder_2_days: c.reminder_2_days ?? null, reminder_3_days: c.reminder_3_days ?? null, create_first_invoice: false }); setShowAddModal(true); }} title="Editar Abonado" className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition"><Edit3 className="w-4 h-4" /></button>
                      <button data-testid={`btn-delete-client-${c.id}`} onClick={() => handleDeleteClient(c.id, c.full_name)} title="Eliminar" className="p-1.5 rounded-lg bg-slate-800 hover:bg-rose-900/40 text-slate-400 hover:text-rose-400 border border-slate-700 transition"><Trash2 className="w-4 h-4" /></button>
                    </div></td>
                  </tr>
                ))
              )}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-4 my-8">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2"><Users className="w-5 h-5 text-cyan-400" />{selectedClient ? "Editar Abonado" : "Nuevo Abonado FibraZ"}</h3>
              <button onClick={() => setShowAddModal(false)} className="text-slate-400 hover:text-slate-200 text-sm font-bold p-1">✕</button>
            </div>
            <form onSubmit={handleSaveClient} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-slate-300 font-semibold mb-1">Nombre Completo / Razón Social *</label>
