...
import { CalendarDays, ClipboardList, MapPinned, MapPin, X } from "lucide-react";
...
export default function NewInstallationModal({ onClose, onRegister }) {
  ...
  const applyCoordinates = (position) => {
    setDraft((current) => ({
      ...current,
  ...
  return <>
    <div className="installation-modal-backdrop fixed inset-0 z-50 flex items-center justify-center bg-transparent p-4">
      <form onSubmit={submit} className="installation-modal w-full max-w-3xl overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl">
        ...
        <div className="grid gap-4 p-6 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <button type="button" onClick={() => setShowMap(true)} className="inline-flex items-center gap-2 rounded-xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-2.5 text-xs font-bold text-cyan-300 transition hover:bg-cyan-500/20">
              <MapPinned className="h-4 w-4" /> Seleccionar ubicación en el mapa
            </button>
            <p className="mt-1 text-[10px] text-slate-500">Abre el mismo minimapa usado en Z-Hub. Haz clic o arrastra el marcador y aplica las coordenadas.</p>
          </div>
    ...
    {showMap && <CoordinatesPicker
      title="Ubicación de la nueva instalación"
