 ...
 * Actualización: 2026-09-08 — versión 1.1.4: advertencia y confirmación explícita antes de eliminar servicios adicionales.
...
const opticalPowerInputClass = (value) => {
  const tone = getOpticalPowerTone(value);
  if (tone === "critical") return "border-rose-500/70 bg-rose-500/10 focus:border-rose-400 focus:ring-rose-500/30";
  if (tone === "warning") return "border-amber-500/70 bg-amber-500/10 focus:border-amber-400 focus:ring-amber-500/30";
  if (tone === "good") return "border-emerald-500/60 bg-emerald-500/5 focus:border-emerald-400 focus:ring-emerald-500/30";
  return "";
};
...
function ServiceModal({ clientId, api, token, service, onSaved, onClose }) {
  ...
  const handleOpticalPowerChange = (e) => { const { value } = e.target; setFormData((p) => ({ ...p, optical_power_dbm: value === "" ? "" : String(-Math.abs(Number(value))) })); };
  const handleZoneChange = (e) => { const { value } = e.target; setFormData((p) => ({ ...p, zone_id: value, nap_box_id: "", nap_port: "" })); };
...
export default function ClientServiceEditor({ clientId, api, token, onSave, onSaveSuccess }) {
  ...
  const deleteService = async (s) => {
    ...
    const serviceDetails = [
      ...
      s.router_name ? `MikroTik: ${s.router_name}` : null,
