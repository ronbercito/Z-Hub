...
function EmptyState({ title, description }) {
  return <div className="rounded-xl border border-dashed border-slate-700 bg-slate-950/40 p-8 text-center"><p className="font-semibold text-slate-300">{title}</p><p className="mt-1 text-sm text-slate-500">{description}</p></div>;
}
function Value({ label, children }) {
  return <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3"><p className="text-[11px] uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 break-words text-sm font-medium text-slate-200">{children || "Sin registrar"}</p></div>;
}
...
export default function ClientDetail({ clientId, api, token, onClose, onClientUpdated }) {
  ...
  const [summaryFormData, setSummaryFormData] = useState({ full_name: "", dni_ruc: "", phone: "", email: "", address: "", reference: "", installation_date: "", zone_id: "", latitude: "", longitude: "" });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setClient(clientData); setZones(zonesRes.data || []);
        setSummaryFormData({ full_name: clientData.full_name || "", dni_ruc: clientData.dni_ruc || "", phone: clientData.phone || "", email: clientData.email || "", address: clientData.address || "", reference: clientData.reference || "", installation_date: clientData.installation_date || "", zone_id: clientData.zone_id || "", latitude: clientData.latitude != null ? String(clientData.latitude) : "", longitude: clientData.longitude != null ? String(clientData.longitude) : "" });
      } catch (err) { if (alive) setError(err.response?.data?.detail || "No se pudo cargar la ficha del cliente."); }
  ...
  const handleSaveSummary = async (e) => {
    ...
    try {
      ...
      if (summaryFormData.latitude || summaryFormData.longitude) {
        const lat = parseFloat(summaryFormData.latitude), lng = parseFloat(summaryFormData.longitude);
        if (Number.isNaN(lat) || Number.isNaN(lng)) throw new Error("Latitud y longitud deben ser números válidos.");
