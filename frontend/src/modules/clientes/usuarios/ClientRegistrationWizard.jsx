...
export default function ClientRegistrationWizard({ selectedClient, formData, setFormData, plans, routers, ipv4Networks, napBoxes, onClose, onSubmit, api, token }) {
  ...
  const submit = (event) => {
    ...
    if (!activePlans.some((plan) => plan.id === formData.plan_id)) {
      toast.error(selectedTechnology === "wireless" ? "Selecciona un plan inalámbrico para este abonado." : "Selecciona un plan de fibra óptica para este abonado.");
      return;
    }
