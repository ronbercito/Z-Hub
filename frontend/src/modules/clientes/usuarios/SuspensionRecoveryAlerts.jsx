...
import { AlertTriangle, Phone, MapPin, Radio, Clock3, RefreshCw } from "lucide-react";
import { useAuth } from "../../../context/AuthContext";

const fmt = (value) => value ? new Date(`${String(value).slice(0,10)}T12:00:00`).toLocaleDateString("es-PE") : "—";

export default function SuspensionRecoveryAlerts() {
      ...
      <div className="divide-y divide-amber-500/10">
        {data.alerts.map((client) => (
          <div key={client.id} className="grid grid-cols-1 gap-3 px-4 py-3 text-xs lg:grid-cols-[1.35fr_1fr_1fr_auto] lg:items-center">
            <div>
              <div className="font-bold text-slate-100">{client.full_name}</div>
              <div className="mt-1 text-slate-500">DNI/RUC: {client.dni_ruc || "—"}</div>
            </div>
            <div className="space-y-1 text-slate-400">
              <div><Phone className="mr-1 inline h-3.5 w-3.5" />{client.phone || "Sin teléfono"}</div>
              <div><MapPin className="mr-1 inline h-3.5 w-3.5" />{client.address || "Sin dirección"}</div>
            </div>
            <div className="space-y-1 text-slate-400">
              <div><Radio className="mr-1 inline h-3.5 w-3.5" />{client.router_name || "Sin router"}{client.onu_sn ? ` · ONU ${client.onu_sn}` : ""}</div>
              <div><Clock3 className="mr-1 inline h-3.5 w-3.5" />Suspendido desde {fmt(client.suspended_at)}</div>
            </div>
            <div className="rounded-xl border border-rose-500/20 bg-rose-500/10 px-3 py-2 text-center font-black text-rose-300">
              {client.months_suspended} {client.months_suspended === 1 ? "mes" : "meses"}{client.extra_days ? ` + ${client.extra_days} d` : ""}
            </div>
          </div>
        ))}
      </div>
