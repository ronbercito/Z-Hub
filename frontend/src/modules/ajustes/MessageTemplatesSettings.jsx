...
import { AlertTriangle, BellRing, Check, CircleCheck, Eye, FileText, MessageSquareText, RotateCcw, Save, Settings2, ShieldAlert, Smartphone, Wrench, Smile } from "lucide-react";
...
export default function MessageTemplatesSettings() {
  ...
  const insertEmoji = (key, emoji) => {
    ...
    const currentText = templates[key]?.text || "";
    const start = textarea?.selectionStart ?? currentText.length;
    const end = textarea?.selectionEnd ?? start;
    const nextText = currentText.slice(0, start) + emoji + currentText.slice(end);
    setTemplates((current) => ({ ...current, [key]: { ...current[key], text: nextText } }));
    setEmojiKey("");
  ...
  return <div className="space-y-6">
    <div className="rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-500/10 via-slate-900/80 to-slate-950 p-5">
      <div className="flex items-start gap-4"><div className="p-3 rounded-2xl bg-violet-500/10 border border-violet-400/20"><MessageSquareText className="w-6 h-6 text-violet-300" /></div><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-black text-slate-100">Centro de plantillas</h3><span className="px-2 py-0.5 rounded-full border border-slate-700 bg-slate-950/50 text-[10px] font-bold text-slate-400">4 plantillas</span></div><p className="text-xs text-slate-400 mt-1.5">Define el lenguaje de las automatizaciones. Usa variables <b className="text-cyan-300">{"{{variable}}"}</b>, marcadores <b className="text-cyan-300">%vip%</b>, formato WhatsApp como <b className="text-slate-200">*negrita*</b> y agrega emojis con el botón de cada plantilla.</p></div></div>
    ...
    <div className="space-y-4">{ORDER.map((key) => { const item = templates[key]; const meta = FLOW[key]; if (!item) return null; const Icon = meta.icon; const open = previewKey === key; const emojiOpen = emojiKey === key; return <div key={key} className={`rounded-2xl border ${TONES[meta.tone]} bg-slate-900/75 overflow-hidden`}>
