...
import { Users, UserPlus, PauseCircle, UserMinus, SlidersHorizontal, ShieldAlert, Save, PackageCheck } from "lucide-react";
...
export default function ClientSettings() {
  ...
  const groups = [
    ...
    },
    {
      ...
      text: "Seguimiento futuro de ONU, router, CPE u otros equipos pendientes de recuperar.",
    },
  ];
      ...
      <section className="client-settings-shell rounded-2xl border border-slate-800 bg-slate-900/90 p-4 shadow-xl">
        <div className="client-settings-shell-header flex items-start gap-3 border-b border-slate-800 pb-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] leading-relaxed text-slate-400">Aquí se concentran las reglas específicas del panel de clientes sin mezclarlas con la configuración general.</p>
          </div>
        </div>

        <div className="client-settings-groups mt-3 grid grid-cols-1 gap-2.5 md:grid-cols-2">
          {groups.map(({ icon: Icon, title, text }) => (
            <div key={title} className="client-settings-group rounded-xl border border-slate-800 bg-slate-950/45 px-3.5 py-3">
