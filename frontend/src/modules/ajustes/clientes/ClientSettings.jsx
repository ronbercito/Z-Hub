...
export default function ClientSettings() {
  ...
  const groups = [
    {
      id: "registration",
      icon: UserPlus,
      title: "Registro y altas",
      text: "Reglas para altas, tecnología, planes y datos iniciales del abonado.",
    },
    {
      id: "pauses",
      icon: PauseCircle,
      title: "Pausas de servicio",
      text: "Políticas de pausa temporal, reactivación y avisos previos.",
    },
    {
      id: "suspensions",
      icon: UserMinus,
      title: "Suspensiones, retiros y reactivaciones",
      text: "Criterios para suspensión prolongada, clientes retirados y retorno al servicio.",
    },
    {
      id: "recovery",
      icon: PackageCheck,
      title: "Recuperación de equipos",
      text: "Seguimiento de ONU, router, CPE u otros equipos pendientes de recuperar.",
    },
  ];
  ...
  const infoPanels = {
    registration: {
      icon: UserPlus,
      title: "Registro y altas",
      description: "Aquí se concentrarán las políticas generales que afecten el alta de nuevos abonados, como valores predeterminados, validaciones y comportamiento del registro.",
      note: "El asistente de registro actual sigue funcionando sin cambios. Esta sección queda habilitada como acceso para agregar sus opciones sin mezclarlas con Facturación ni Ajustes generales.",
    },
    pauses: {
      icon: PauseCircle,
      title: "Pausas de servicio",
          ...
          {groups.map(({ id, icon: Icon, title, text }) => {
            ...
            return (
              <button
                ...
                aria-pressed={active}
                className={`client-settings-group client-settings-group-button rounded-xl border px-3.5 py-3 text-left transition ${active ? "client-settings-group-active border-cyan-500/60 bg-cyan-500/10" : "border-slate-800 bg-slate-950/45 hover:border-cyan-500/40 hover:bg-slate-900"}`}
              >
        ...
        <section className="client-settings-policy rounded-2xl border border-amber-500/20 bg-slate-900/90 p-4 shadow-xl">
          ...
          {loading ? <div className="py-5 text-sm text-slate-500">Cargando configuración…</div> : <div className="mt-3 space-y-3">
            <label className="client-settings-toggle flex items-center justify-between gap-4 rounded-xl border border-slate-800 bg-slate-950/45 px-3.5 py-3">
              <div>
                <div className="text-[13px] font-bold text-slate-200">Activar alerta</div>
                <div className="mt-1 text-[11px] text-slate-500">Solo genera un aviso visual. No retira ni elimina al cliente automáticamente.</div>
              </div>
            ...
            <div className="client-settings-note rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2.5 text-[11px] leading-relaxed text-amber-200">
              Cuando se cumpla el plazo, el aviso permanecerá visible y mostrará cuánto tiempo lleva suspendido el cliente. La decisión de retirarlo o recuperar equipos sigue siendo manual.
            </div>
          </div>}
        </section>
      ) : renderInfoPanel()}
