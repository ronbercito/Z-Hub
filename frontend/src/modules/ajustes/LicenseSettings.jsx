...
export default function LicenseSettings() {
  ...
  const requestLabel = isTrial ? "Solicitar licencia" : "Renovar licencia";
  const whatsappMessage = [
    `Hola, deseo ${isTrial ? "pasar mi Z-Hub Trial a una licencia pagada" : "renovar o revisar mi licencia Z-Hub"}.`,
  ...
  const expiry = isTrial ? data?.trial_expires_at : data?.license_expires_at;
  ...
  return <div className="license-settings">
    ...
    {requiresCommercialHelp && <section className="license-recovery-lock">
      ...
      <div>
        <b>{data?.status === "trial_expired" ? "Tu período de prueba finalizó" : "Esta instalación necesita una licencia activa"}</b>
        <p>Tus datos permanecen intactos. Solicita la activación o renovación por WhatsApp y, cuando el cambio se realice en Web-Licence, pulsa “Actualizar estado”.</p>
      </div>
    </section>}
    ...
    <section className="license-hero">
      <div>
        ...
        <p>{isTrial ? "Prueba de 30 días con capacidad máxima de 20 abonados." : "Licencia administrada centralmente desde Web-Licence."}</p>
      </div>
    </section>

    <section className="license-metrics">
      <article><span><ShieldCheck/>Plan</span><strong>{planLabel(data?.plan, max)}</strong></article>
      <article><span><Clock3/>Vencimiento</span><strong>{dateLabel(expiry)}</strong></article>
      <article><span>{unlimited ? <InfinityIcon/> : <Users/>}Capacidad</span><strong>{unlimited ? "Ilimitada" : (max ?? "—")}</strong></article>
    </section>

    {isTrial && <section className={`license-trial-card ${data?.status === "trial_expired" ? "expired" : ""}`}>
      <div className="trial-icon"><Clock3/></div>
      <div><span>Días restantes del Trial</span><strong>{remainingDays} días</strong><p>El mismo hardware no recibe un segundo período de prueba al reinstalar Z-Hub.</p></div>
    </section>}

    {!unlimited && max != null && <section className="license-capacity">
      <div className="capacity-row"><b>Uso de capacidad</b><span>{usage} / {max} abonados · {percent}%</span></div>
      <div className="capacity-track" aria-label={`Uso de licencia ${percent}%`}><span style={{width:`${percent}%`}} /></div>
      <p>{available > 0 ? `${available} cupo${available === 1 ? " disponible" : "s disponibles"}.` : "Capacidad autorizada alcanzada."}</p>
    </section>}
    ...
    <section className="license-details">
      ...
      <div><span>Vencimiento</span><b>{dateLabel(expiry)}</b></div>
    ...
    <section className="license-commercial">
      <div className="commercial-title"><MessageCircle/><div><b>{requestLabel}</b><span>La activación, cambio de plan y renovación se gestionan directamente con soporte.</span></div></div>
      <div className="commercial-actions">
        <button type="button" className="whatsapp" disabled={!whatsappUrl} onClick={() => whatsappUrl && window.open(whatsappUrl, "_blank", "noopener,noreferrer")}><MessageCircle/>{requestLabel} por WhatsApp</button>
      </div>
    ...
    <footer className="license-note">La licencia se administra desde Web-Licence. En Z-Hub no es necesario copiar ni ingresar claves manualmente.</footer>
  </div>;
}
