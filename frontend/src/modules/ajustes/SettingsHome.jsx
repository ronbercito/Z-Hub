...
const MODULES = [
  ...
  { id:"config_templates", label:"Plantillas configuración", desc:"Ajustes reutilizables", icon:LayoutTemplate, tone:"violet" },
...
export default function SettingsHome({ onOpen }) {
  const live = MODULES.filter((item) => item.live).length;
  const developing = MODULES.length - live;
  return <div className="settings-home">
    <div className="settings-home-head">
      <div>
        <h2><Settings /> Ajustes</h2>
        <p>Configura cada módulo del sistema. Pasa el mouse sobre una opción para ver su estado.</p>
      </div>
      <div className="settings-summary">
        <div className="summary-pill ok"><CheckCircle2/><b>{live}</b><span>Módulos operativos<small>Funcionando actualmente</small></span></div>
        <div className="summary-pill dev"><Hammer/><b>{developing}</b><span>En desarrollo<small>Interfaz preparada</small></span></div>
        <div className="summary-pill err"><AlertTriangle/><b>0</b><span>Con errores<small>Sin errores declarados</small></span></div>
      </div>
    </div>

    <div className="settings-module-grid">
      {MODULES.map(({ id,label,desc,icon:Icon,tone,live }) => <button key={id} type="button" className={`settings-module-card tone-${tone} ${live ? "is-live" : "is-dev"}`} onClick={() => onOpen?.(id)} aria-label={`${label}: ${live ? "Operativo" : "En desarrollo"}`}>
        <span className="module-icon"><Icon /></span>
        <strong>{label}</strong>
        <span className={`module-state ${live ? "state-live" : "state-dev"}`}>{live ? <CheckCircle2/> : <Hammer/>}{live ? "Operativo" : "En desarrollo"}</span>
        <span className="module-tooltip"><b>{label}</b><span>{desc}</span><em>{live ? "Operativo" : "En desarrollo"}</em></span>
      </button>)}
    </div>

    <div className="settings-legend"><span><i className="dot live"/>Operativo: función disponible</span><span><i className="dot dev"/>En desarrollo: acceso preparado</span><span>El estado es informativo y no modifica ninguna configuración.</span></div>
  </div>;
}
