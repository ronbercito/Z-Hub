...
def parse_vsol_uplink_traffic(page: str) -> dict[str, Any]:
    """Extrae los contadores RX/TX del puerto GE que está enlazado (Up)."""
    ...
    for row in parser.rows:
        if len(row) < 9 or not re.fullmatch(r"GE\d+", row[0].strip(), re.I):
            continue
        try:
            rx_bytes = int(re.sub(r"[^0-9]", "", row[3]) or "0")
            tx_bytes = int(re.sub(r"[^0-9]", "", row[8]) or "0")
        except ValueError:
            continue
        ports.append({
            "port": row[0].strip().upper(),
            "link_status": row[1].strip().lower(),
            "speed": row[2].strip(),
            "rx_bytes": rx_bytes,
            "tx_bytes": tx_bytes,
        })

    active = next((item for item in ports if item["port"] == "GE4" and item["link_status"] == "up"), None)
    active = active or next((item for item in ports if item["link_status"] == "up"), None)
    if not active:
        raise OltWebError("La web VSOL no reportó ningún puerto GE activo.")

    return {**active, "ports": ports}


def _fetch_vsol_uplink_traffic(
    host: str,
    username: str,
    password: str,
    web_port: int = 443,
) -> dict[str, Any]:
    """Inicia sesión web y lee /action/geinfo.html sin realizar cambios."""
    ...
    login_url = f"{base_url}/action/login.html"
    main_url = f"{base_url}/action/main.html"
    traffic_url = f"{base_url}/action/geinfo.html"
    payload = urlencode({"user": username, "pass": password or "", "button": "Login", "who": "100"}).encode("utf-8")
    ...
    headers = {"Accept": "text/html,application/xhtml+xml", "User-Agent": "MikroHub-OLT-Monitor/1.0"}
    ...
    try:
        ...
        opener.open(
            Request(
                main_url,
                data=payload,
                headers={**headers, "Content-Type": "application/x-www-form-urlencoded", "Origin": base_url, "Referer": login_url},
                method="POST",
            ),
            timeout=TIMEOUT + 3,
        ).read()
        ...
        raise OltWebError(f"No se pudo leer el tráfico web de la OLT: {exc}") from exc

    return parse_vsol_uplink_traffic(page)


async def get_vsol_web_uplink_traffic(router: Any) -> dict[str, Any]:
    """Obtiene contadores acumulados del uplink VSOL sin bloquear FastAPI."""

    traffic = await asyncio.to_thread(
        _fetch_vsol_uplink_traffic,
        str(getattr(router, "ip_address", "") or "").strip(),
        str(getattr(router, "web_username", "") or "").strip(),
        str(getattr(router, "web_password", "") or ""),
        int(getattr(router, "web_port", 443) or 443),
    ...
    return {
        "ok": True,
        "message": "Tráfico uplink VSOL actualizado",
        "error": "",
        "raw": f'Puerto: {traffic["port"]}\nRX Bytes: {traffic["rx_bytes"]}\nTX Bytes: {traffic["tx_bytes"]}',
        "info": traffic,
        "rows": traffic["ports"],
        "commands": ["HTTPS GET /action/geinfo.html"],
        "source": "vsol_web_traffic",
    }
