"""
Archivo: backend/app/routers/clientes/installations.py
Función: CRUD mínimo de solicitudes de instalación previas al alta del abonado.
No crea clientes ni altera aprovisionamiento; únicamente guarda/lista/elimina
...
async def list_installations(db: AsyncSession = Depends(get_db)):
    rows = await db.scalars(
        select(Installation)
        .where(Installation.status == "pending")
...
async def create_installation(
    ...
    installation = Installation(
        ...
        latitude=float(payload.latitude or 0),
        longitude=float(payload.longitude or 0),
        installation_date=payload.installation_date or "",
        technology=payload.technology or "fiber",
        status="pending",
