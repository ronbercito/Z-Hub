...
def _eligibility(kind, invoice, client, automation, today):
    ...
    if kind == "reminder":
        ...
        eligible = invoice.status in {"unpaid", "pending"} and days == configured
        return eligible, f"Cumple el recordatorio configurado a {configured} días" if eligible else f"Hoy faltan {days} días; la configuración requiere {configured}", days
    grace = max(0, int(client.grace_days or 0))
    overdue = (today - due).days
    eligible = invoice.status in {"unpaid", "pending", "overdue"} and due < today and overdue >= grace
    return eligible, f"Cumple el aviso de corte: {overdue} días vencidos y {grace} días de gracia" if eligible else f"Tiene {overdue} días vencidos y {grace} días de gracia", days
...
def _message(kind, setting, client, invoice):
    ...
    phone = data.get("phone") or yape
    titular_pago = data.get("yape_holder") or data.get("yape_name") or ""
    kwargs = {"company": company, "yape": yape, "phone": phone, "titular_pago": titular_pago, "cutoff_date": invoice.due_date}
    if kind == "reminder":
        return payment_reminder(templates["payment_reminder"]["text"], client.full_name, invoice.amount, invoice.plan_name or client.plan_name, invoice.due_date, **kwargs)
    if kind == "cut":
        return cut_warning(templates["cut_warning"]["text"], client.full_name, invoice.amount, **kwargs)
    return payment_confirmation(templates["payment_confirmation"]["text"], client.full_name, invoice.paid_amount or invoice.amount, invoice.invoice_number, **kwargs)
...
async def candidates(kind: Kind = Query(...), limit: int = Query(default=50, ge=1, le=100), db: AsyncSession = Depends(get_db)):
    ...
    cfg = _root_config(setting)
    today = date.today()
    query = select(Invoice).where(Invoice.status.in_(["paid", "pagado"] if kind == "payment" else ["unpaid", "pending", "overdue"])).limit(500)
...
async def send_test(data: TestInvoiceRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not eligible and not data.force_test:
        raise HTTPException(status_code=409, detail=f"La factura no cumple la condición automática: {reason}. Para una prueba controlada use la opción de prueba forzada.")
    message = _message(data.kind, setting, client, invoice)
    phone = client.phone or invoice.client_phone
    try:
        result = await send_messages(api_key=cfg["api_key"], contacts=[{"number": phone, "message": message}], gateway_url=cfg.get("gateway_url"), country_code=cfg.get("country_code", "51"), verify=bool(cfg.get("verify", True)))
