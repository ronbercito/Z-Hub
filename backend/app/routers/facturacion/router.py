...
Actualización: 2026-09-08 — aplicación automática de saldos del cliente a facturas nuevas y cálculo correcto del pendiente.
...
Trabaja con: backend/app/models/invoice.py, client.py, client_service.py, client_balance.py,
...
async def _refresh_balance(db: AsyncSession, client: Client):
    ...
    client.balance_due = round(
        sum(max(0.0, float(i.amount or 0) - float(i.paid_amount or 0)) for i in pending),
        2,
    )
    return pending
...
async def _service_labels(db: AsyncSession, client_ids: set[str]) -> dict[tuple[str, str], str]:
    ...
    rows = (await db.execute(
        ...
        .where(ClientService.client_id.in_(client_ids))
...
async def create_invoice(data: InvoiceIn, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    inv = Invoice(
        invoice_number=data.invoice_number or correlative("REC"),
        client_id=c.id,
        service_id=service.id if service else None,
        client_name=c.full_name,
        client_dni_ruc=c.dni_ruc,
...
async def register_payment(data: PaymentIn, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "invoice": (await _decorate_invoices(db, [inv]))[0],
        "mikrotik": mikrotik,
    }
...
async def mass_generate(db: AsyncSession = Depends(get_db)):
    ...
    for c in clients:
        ...
        inv = Invoice(
            invoice_number=correlative("REC"),
            client_id=c.id,
            service_id=None,
            client_name=c.full_name,
            client_dni_ruc=c.dni_ruc,
            ...
            status="unpaid",
            notes=f"Factura mensual periodo {period} - Servicio 1",
        )
        db.add(inv)
        await db.flush()
