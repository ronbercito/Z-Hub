...
async def _refresh_balance(db: AsyncSession, client: Client):
    unpaid = (await db.execute(
        select(Invoice).where(Invoice.client_id == client.id, Invoice.status.in_(["unpaid", "overdue"]))
    )).scalars().all()
...
async def _service_labels(db: AsyncSession, client_ids: set[str]) -> dict[tuple[str, str], str]:
    ...
    rows = (await db.execute(
        select(ClientService).where(ClientService.client_id.in_(client_ids))
        .order_by(ClientService.client_id.asc(), ClientService.created_at.asc(), ClientService.id.asc())
    )).scalars().all()
    labels = {}
    counters = {}
    for row in rows:
...
async def create_invoice(data: InvoiceIn, db: AsyncSession = Depends(get_db)):
    ...
    inv = Invoice(
        invoice_number=data.invoice_number or correlative("REC"), client_id=c.id,
        service_id=service.id if service else None, client_name=c.full_name,
...
async def register_payment(data: PaymentIn, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if data.amount > inv.amount:
        raise HTTPException(status_code=400, detail="El pago no puede superar el monto de la factura")
    ...
    await db.commit()
    invoice_data = (await _decorate_invoices(db, [inv]))[0]
    return {"message": "Pago registrado exitosamente. Recibo emitido.", "invoice": invoice_data, "mikrotik": mikrotik}
...
async def mass_generate(db: AsyncSession = Depends(get_db)):
    ...
    count = 0
    for c in clients:
        exists = (await db.execute(select(func.count()).select_from(Invoice).where(
            Invoice.client_id == c.id, Invoice.month_period == period, Invoice.service_id.is_(None)
        ))).scalar()
        if exists or not c.plan_price:
            continue
        due = now.replace(day=min(max(c.billing_day, 1), 28)) + timedelta(days=5)
        db.add(Invoice(invoice_number=correlative("REC"), client_id=c.id, service_id=None,
                       client_name=c.full_name, client_dni_ruc=c.dni_ruc, client_address=c.address,
                       client_phone=c.phone, plan_name=c.plan_name, amount=c.plan_price,
