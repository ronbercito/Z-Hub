...
def test_license_server_exposes_admin_web_and_customer_crud():
    ...
    assert 'APP_VERSION = "1.2.0"' in main
...
def test_license_center_automates_keys_plans_and_customer_data():
    ...
    assert 'secrets.token_hex(4).upper()' in main
...
def test_trial_expiry_and_revoked_status_are_server_side():
    main = read("license_server/app/main.py")
...
def test_panel_version_marks_1268():
    version = read("frontend/src/modules/system-update/version.js")
    assert 'PANEL_VERSION = "1.2.68"' in version
    assert "Claves automáticas" in version
