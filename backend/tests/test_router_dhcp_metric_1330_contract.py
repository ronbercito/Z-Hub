...
def test_release_1330_is_documented():
    version = read("frontend/src/modules/system-update/version.js")
    continuity = read("docs/CONTINUIDAD_Z-HUB-v1.3.md")
    assert 'PANEL_VERSION = "1.3.30"' in version
    assert "1.3.30 — DHCP en tarjeta MikroTik" in continuity
    assert "backup/pre-router-dhcp-metric-1.3.30-20260912" in continuity
